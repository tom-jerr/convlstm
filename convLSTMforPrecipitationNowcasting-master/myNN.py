    ...
    def forward(self, input, hidden_states = None):
        ...
        if hidden_states == None:
            ...
            for i in range(self.layer_numbers):
                hidden_state_i = (torch.tensor(torch.zeros(batch_size, self.hidden_channels[i], self.shape[0], self.shape[1])),
                                  torch.tensor(torch.zeros(batch_size, self.hidden_channels[i], self.shape[0], self.shape[1])))
...
if __name__ == '__main__':
    ...
    xtrain = torch.tensor(torch.randn(3, 100, 3, 28, 28))
    ytrain = torch.tensor(torch.randn(100, 5, 28, 28))
